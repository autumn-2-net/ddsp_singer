...
def linear_beta_schedule(timesteps, config):
    ...
    linear schedule
    """
    max_beta = config.get('max_beta', 0.01)
    betas = np.linspace(1e-4, max_beta, timesteps)
    return betas


def cosine_beta_schedule(timesteps, config):
    """
    cosine schedule
    as proposed in https://openreview.net/forum?id=-NEXDKk8gZ
    """
    s=config.get('cosine_beta_schedule_s', 0.008)
    steps = timesteps + 1
...
class GaussianDiffusion(nn.Module):
    def __init__(self, out_dims,config, num_feats=1, timesteps=1000, k_step=1000,
        ...
        if betas is not None:
            ...
            betas = beta_schedule[self.config['schedule_type']](timesteps,self.config)

        alphas = 1. - betas
        alphas_cumprod = np.cumprod(alphas, axis=0)
        alphas_cumprod_prev = np.append(1., alphas_cumprod[:-1])

        self.use_shallow_diffusion = self.config.get('use_shallow_diffusion', False)
    ...
    def inference(self, cond,promot, b=1, x_start=None, device=None):
        depth = self.config.get('diff_depth', self.k_step)
        ...
        if self.config.get('pndm_speedup') and self.config['pndm_speedup'] > 1 and t_max > 0:
            algorithm = self.config.get('diff_accelerator', 'ddim')
            if algorithm == 'dpm-solver':
                ...
                steps = t_max // self.config["pndm_speedup"]
                ...
                self.noise_list = deque(maxlen=4)
                iteration_interval = self.config['pndm_speedup']
                for i in tqdm(
