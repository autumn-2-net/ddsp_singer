...
class DiffusionNoiseLoss(nn.Module):
    ...
    def _mask_nonpadding(x_recon, noise, nonpadding=None):
        if nonpadding is not None:
            ...
            return x_recon.masked_fill(~nonpadding.unsqueeze(1).unsqueeze(1), 0), noise.masked_fill(
                ~nonpadding.unsqueeze(1).unsqueeze(1), 0)
        else:
...
class ddsp_ss(BaseTask):
    def __init__(self, config):
        ...
        self.train_dataset = SVS_Dataset(paths=str(pathlib.Path(config['data_index_path']) / 'train_svs'),
                                         config=config, vocab_list=vbc)
        self.valid_dataset = SVS_Dataset(paths=str(pathlib.Path(config['data_index_path']) / 'val_svs'), config=config,
                                         vocab_list=vbc)
    ...
    def run_model(self, sample, infer=False, inx=False):
        ...
        target = sample.get('gtmel')  # [B, T_s, M]
        ...
        if infer:
            ...
            if wavmask is not None:
                ...
                loss, (loss_rss, loss_uv) = self.wav_loss(signal * wavmask.long(), s_h * wavmask.long(),
                                                          wav * wavmask.long(), (uv).float(),
                                                          detach_uv=detach_uv,
                                                          uv_tolerance=0.05)
            else:
    ...
    def _validation_step(self, sample, batch_idx):
        ...
        if batch_idx < self.config['num_valid_plots'] \
            ...
            with torch.no_grad():
                ...
                self.logger.experiment.add_audio(f'diff_{batch_idx}_', wav_out,
    ...
    def plot_wav(self, batch_idx, gt_mel, typess, aux_mel=None, diff_mel=None, f0=None):
        ...
        if batch_idx not in self.logged_gt_wav:
            gt_wav = self.vocoder.spec2wav(gt_mel, f0=f0)
            self.logger.experiment.add_audio(f'gt_{batch_idx}_{typess}', gt_wav,
                                             sample_rate=self.config['audio_sample_rate'],
                                             global_step=self.global_step)
            self.logged_gt_wav.add(batch_idx)
        if aux_mel is not None:
            aux_wav = self.vocoder.spec2wav(aux_mel, f0=f0)
            self.logger.experiment.add_audio(f'aux_{batch_idx}_{typess}', aux_wav,
                                             sample_rate=self.config['audio_sample_rate'],
                                             global_step=self.global_step)
        if diff_mel is not None:
            diff_wav = self.vocoder.spec2wav(diff_mel, f0=f0)
            self.logger.experiment.add_audio(f'diff_{batch_idx}_{typess}', diff_wav,
                                             sample_rate=self.config['audio_sample_rate'],
                                             global_step=self.global_step)
